...
INTERFACE::INTERFACE(const string vala){
	this->inputSv  = "";
	this->sockAddr = vala;
	this->port     = 54000;
}
INTERFACE::INTERFACE(const int vala){
	this->inputSv  = "";
	this->sockAddr = "0.0.0.0";
	if(vala > 65535) {
		this->port = 54000;
...
INTERFACE::INTERFACE(const string vala, const int valb){
    this->inputSv  = "";
    this->sockAddr = vala;
    this->port     = valb;
}
string INTERFACE::currentDateTime() {
    time_t     now = time(0);
    struct tm  tstruct;
    char       buf[80];
...
void INTERFACE::setMiddleFace(SERVER *middle_face){
	this->middle_face = middle_face;
}
bool INTERFACE::checkConnection(){
    bool status = false;
...
void INTERFACE::operateSession(const string input_string){
...
void INTERFACE::disconnectSession(){
    if(this->current_address != ""){
